        ...
        return {
            ...
            link: function($scope, elem, attrs) {
                ...
                $scope.processResults = function(responseData, str) {
                    if (responseData && responseData.length > 0) {
                        ...
                        for (var i = 0; i < responseData.length; i++) {
                            ...
                            var count = "";
                            if ($scope.businessCount) {
                                count = responseData[i][$scope.businessCount];
                            }
                ...
                $scope.searchTimerComplete = function(str) {
                    ...
                    if (str.length >= $scope.minLength) {
                        if ($scope.localData) {
                            ...
                            $scope.strValue = str;
                            $http.jsonp($scope.url + str, {params:sendDetail}).
                            success(function(responseData, status, headers, config) {
                                $scope.searching = false;
                                var resData = responseData.body.result.deal_list;
                                if (responseData.body.result.business_count) {
                                    $scope.businessList = responseData.body.result.business_count;
                                };
                                
                                $scope.processResults((($scope.dataField) ? responseData[$scope.dataField] : resData), str);
                            }).
                ...
                $scope.selectResult = function(result, event) {
                    ...
                    // 选择项做跳转
                    $location.path('item/' + result.originalObject.deal_id)
                ...
                inputField.on('input', $scope.keyPressed);
